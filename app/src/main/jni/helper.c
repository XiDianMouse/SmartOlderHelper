 ...
 */
void check_and_restart_service(char* srvname) {
	//LOGI("当前所在的进程pid=",getpid());
	char cmdline[200];
	sprintf(cmdline, "am startservice --user 0 -n %s", srvname);
	char tmp[200];
 ...
 * 返回ABI给Java
 */
jstring Java_com_smartolderhelper_daemon_NativeRuntime_stringFromJNI(JNIEnv* env, jobject thiz) {
	#if defined(__arm__)
	...
	return (*env)->NewStringUTF(env,
			"GBH Hello from JNI !  Compiled with ABI " ABI ".");
}
 ...
 * 查找进程
 */
pid_t JNICALL Java_com_smartolderhelper_daemon_NativeRuntime_findProcess(JNIEnv* env,
 ...
 * 启动Service
 */
void Java_com_smartolderhelper_daemon_NativeRuntime_startService(JNIEnv* env, jobject thiz,
 ...
 * 关闭Service
 */
void Java_com_smartolderhelper_daemon_NativeRuntime_stopService() {
	exit(0);
}
 ...
 * 启动Activity
 */
void Java_com_smartolderhelper_daemon_NativeRuntime_startActivity(JNIEnv* env, jobject thiz,
